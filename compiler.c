...
void next() {
	...
	while(token = *src) {
		...
		if (token == '\n') {
			...
			src++;
			//if it's a single character, return Num token
			if(token == '"') {
...
void function_body() {
	...
	while(token == Int || token == Char) {
		//local variable declarations, just like global ones.
		...
		if(current_id[Class] == Loc) {
			//identifier exists
			printf("%d: duplicate local declaration", line);
			exit(-1);
		}
		...
		current_id[BClass] = current_id[Class];
		current_id[Class] = Loc;
		current_id[BClass] = current_id[Type];
		current_id[Type] = type;	
		current_id[BValue] = current_id[Value];
...
void global_declaration() {
	//global_declaratio ::= enum_decl | variable_decl | function_decl
	//
...
int eval() {
	...
	while(1) {
		...
		//can't understand move instruction
		if(op == IMM)			{ax = *pc++;}								//load immediate value to ax
		else if(op == LC) 		{ax = *(char *)ax;}				//load character to ax, address in ax
		else if(op == LI)		{ax = *(int *)ax;}				//load integer to ax, address in ax
		else if(op == SC) 		{ax = *(char *)*sp++ = ax;}		//save character to address, value in ax, address pn stack
		...
		else if(op == LEA)		{ax = (int)(bp + *pc++);}					//load address for argumentsi
		//operator
